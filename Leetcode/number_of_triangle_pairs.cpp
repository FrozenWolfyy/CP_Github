...
int countZeroTriplets(vector<int> a){
      int n=a.size();
      int res=0;
      for(int i=0;i<n;i++){
            while(a[i]==0){
            ...
            while(start<=end){
                  ...
                  if(a[start] + target > a[end] && a[end]- a[start] > target && a[end]-target>a[start]){
                        res+=1;
